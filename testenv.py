...
class snakeenv(gym.Env):
    ...
    def step(self, action):
        ...
        # Display Apple
        cv2.circle(self.img, (self.apple_position[0]+5, self.apple_position[1]+5), 5, (0, 0, 255), cv2.FILLED)
        # Display Snake
        for i, position in enumerate(self.snake_position):
            color = (0, 255, 255) if i == 0 else (0, 255, 0)  # Yellow for head, green for body
            cv2.rectangle(self.img, (position[0], position[1]), (position[0] + 10, position[1] + 10),
                          color, cv2.FILLED)
       
        cv2.putText(self.img, 'score :'+str(self.score), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), thickness=2)
